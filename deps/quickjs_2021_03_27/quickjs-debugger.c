...
void* delay_attempted(JSDebuggerInfo* info) {
    // 在这个线程里，我们只是简单地睡眠1秒
    sleep(1);
    info->attempted_connect = 0;
    return NULL;
}
    ...
    info->ctx = ctx;
     if (!info->attempted_connect) {
         ...
         if (address != NULL && !info->transport_close) {
             int res = js_debugger_connect(ctx, address);
             if (res == 0){
                 pthread_t thread;
                 pthread_create(&thread, NULL, (void *(*)(void *)) delay_attempted, info);
